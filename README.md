...
И так, путем долгих расчет пришел к заключению, что верный ответ 
- A) 2001:DB8:2314:5::/52
  Поскольку у нас маска 52, то первые 52 бита используются для префикса.
  Каждый 16тиричный символ содержит 2 бита.Получается что каждый блок содержит 2*4=16 бит
  52/16=3.25, то есть 3 целых блока совпадает и первый символ из четвертого
